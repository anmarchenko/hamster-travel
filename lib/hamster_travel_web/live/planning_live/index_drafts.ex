defmodule HamsterTravelWeb.Planning.IndexDrafts do
  ...
  def render(assigns) do
    ...
    <.container wide>
      ...
      <div :if={!@empty_state?} class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <form
          id="drafts-search-form"
          phx-change="search"
          phx-submit="search"
          class="sm:ml-auto w-full sm:max-w-sm"
        >
          <label for="drafts-search-input" class="sr-only">{gettext("Search drafts")}</label>
          <input
  ...
  def handle_params(params, _url, socket) do
    ...
    paginated_drafts =
      Planning.list_drafts_paginated(socket.assigns.current_user, page, @page_size, search_query)

    socket =
      socket
      |> assign(empty_state?: paginated_drafts.total_entries == 0 and is_nil(search_query))
