defmodule HamsterTravelWeb.Planning.DayExpenseNew do
  @moduledoc """
  Button to add a new day expense.
  """
  ...
  def render(%{edit: true} = assigns) do
    ...
    <div class={@class} id={@id}>
      <.live_component
        ...
        id={"day-expense-form-new-#{@id}"}
        trip={@trip}
        day_index={@day_index}
        action={:new}
        can_edit={@can_edit}
        on_finish={fn -> send(self(), {:finish_adding, "day-expense"}) end}
      />
    </div>
    """
  end
  ...
  def handle_event("add_day_expense", _, socket) do
    if socket.assigns.can_edit do
      send(self(), {:start_adding, "day-expense", socket.assigns.id})
      {:noreply, socket}
    else
