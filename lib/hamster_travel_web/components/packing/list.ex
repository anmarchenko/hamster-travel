      ...
      <span>
        <UI.card>
          <div class="flex flex-col w-full">
            <div class="p-4 bg-violet-700 text-white rounded-t-lg">
              <%= @list.name %>
            </div>
            <div class="px-4 py-2">
              <%= for item <- @list.items do %>
                <div class="mt-2">
                  <%= label class: "cursor-pointer inline-flex items-center block gap-2" do %>
                    <%= checkbox(:items, "item-#{item.id}", value: item.checked, class: "accent-violet-700 border-gray-300 rounded w-5 h-5 ease-linear transition-all duration-150 dark:bg-gray-800 dark:border-gray-300") %>
